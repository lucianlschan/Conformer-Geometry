...
def get_rmsd(source_1, source_2, file_in, seed):
    ...
    bo_data = pd.DataFrame({"target": bo_target, "Uniform": bo_rmsd_uniform, "EI": bo_rmsd_EI, "LCB": bo_rmsd_LCB, "Confab": bo_rmsd_confab, "N_rot":5}, columns = ["target", "Uniform", "EI", "LCB", "Confab", "N_rot"])
    confab_data = pd.DataFrame({'target': confab_target, "Uniform": confab_rmsd_uniform, "EI": confab_rmsd_EI, "LCB": confab_rmsd_LCB, "N_rot":5}, columns = ["target","Uniform","EI","LCB", "N_rot"])   
...
output = get_rmsd(source_1, source_2, file_in, 321)
output[0].to_csv(os.path.dirname(os.getcwd()) + '/result/rmsd/five_confab_rmsd_ref.txt', index=False)
output[1].to_csv(os.path.dirname(os.getcwd()) + '/result/rmsd/five_bo_rmsd_ref.txt', index=False)
